...
def stammbestimmen(vocabel):
    for i in voc.keys():
        for j in range(len(voc[i][0])-1):
            if vocabel[:2] == voc[i][0][j][:2]:
                return i
...
assert stammbestimmen("laudo") == "laudare"
assert übersetzung("laudare") == ['loben', 'preisen', 'anpriesen', 'gutheißen', 'rühmen']